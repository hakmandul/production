import { _ as __nuxt_component_0$1 } from './nuxt-link-CL8d-1v6.mjs';
import { mergeProps, defineComponent, withCtx, createTextVNode, unref, createVNode, createBlock, toDisplayString, openBlock, useSSRContext } from 'vue';
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              if (unref(session).user.image) {
                ...
                _push2(`<div class="h-9 w-9 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold group-hover:ring-2 group-hover:ring-blue-500 transition"${_scopeId}>${ssrInterpolate(unref(session).user.name?.charAt(0)?.toUpperCase() ?? "U")}</div>`);
              }
            } else {
              return [
                ...
                }, toDisplayString(unref(session).user.name?.charAt(0)?.toUpperCase() ?? "U"), 1))
              ];
            }
          }),
          _: 1
...
//# sourceMappingURL=default-2sfDQei1.mjs.map

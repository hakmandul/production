import { _ as __nuxt_component_0$1 } from './nuxt-link-_FxAmZUT.mjs';
import { mergeProps, defineComponent, withAsyncContext, withCtx, createTextVNode, unref, createVNode, createBlock, toDisplayString, openBlock, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderSlot, ssrInterpolate, ssrRenderAttr } from 'vue/server-renderer';
import { _ as _export_sfc, a as authClient, b as useRequestHeaders } from './server.mjs';
import { u as useAsyncData } from './asyncData-K7ZdKEFQ.mjs';
import '../../index.mjs';
import 'node:events';
import 'node:buffer';
import 'node:fs';
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: session } = ([__temp, __restore] = withAsyncContext(async () => useAsyncData("header-session", async () => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(session)) {
        ...
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="hidden sm:block text-sm font-medium text-gray-700 group-hover:text-blue-600"${_scopeId}>${ssrInterpolate(unref(session).user.name)}</span>`);
              if (unref(session).user.image) {
                _push2(`<img class="h-9 w-9 rounded-full border border-gray-300 group-hover:ring-2 group-hover:ring-blue-500 transition"${ssrRenderAttr("src", unref(session).user.image)} alt="Avatar"${_scopeId}>`);
              } else {
                _push2(`<div class="h-9 w-9 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold group-hover:ring-2 group-hover:ring-blue-500 transition"${_scopeId}>${ssrInterpolate(unref(session).user.name?.charAt(0)?.toUpperCase() ?? "?")}</div>`);
              }
            } else {
              return [
                createVNode("span", { class: "hidden sm:block text-sm font-medium text-gray-700 group-hover:text-blue-600" }, toDisplayString(unref(session).user.name), 1),
                unref(session).user.image ? (openBlock(), createBlock("img", {
                  key: 0,
                  class: "h-9 w-9 rounded-full border border-gray-300 group-hover:ring-2 group-hover:ring-blue-500 transition",
                  src: unref(session).user.image,
                  alt: "Avatar"
                ...
                }, toDisplayString(unref(session).user.name?.charAt(0)?.toUpperCase() ?? "?"), 1))
              ];
            }
          }),
          _: 1
...
//# sourceMappingURL=default-IKH7A6Us.mjs.map
